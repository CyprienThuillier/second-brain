DOM-based XSS arises when JavaScript take data from an attacker-controllable source, such as the URL, and passes it to a sink that supports dynamic code execution, such as `eval()` or `innerHTML`. This enables attackers to execute malicious JavaScript, which typically allows them to hijack other users' accounts.

To deliver a DOM-based XSS attack, you need a place data into a source so that it is propagated to a sink and causes execution of arbitrary JavaScript.
...
Note that browsers behave differently with regards to URL-encoding, Chrome, Firefox and Safari will URL-encode location.search and location.hash, while IE11 and Microsoft Edge will not URL-encode these sources. If your data gets URL-encoded before being processed, then an XSS attack is unlikely to work. 
